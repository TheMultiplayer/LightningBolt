    ...
    public NetworkListener() {
        try {
            ...
            if (Bolt.manager.getTeamNumber() == 0) {
                System.out.println(
                    "[network]  Team number not set.  Reverting to localhost and enabling local NetworkTables server.");
                NetworkTable.setServerMode();
                NetworkTable.setIPAddress("127.0.0.1");
            } else {
                System.out.println("[network]  Set team number to " +
                                   Bolt.manager.getTeamNumber());
            }
        } catch (Exception e) {
            System.err.println(
                "Connection failed.  Reverting to localhost.  No server will be started.");
            NetworkTable.setIPAddress("127.0.0.1");
        };
